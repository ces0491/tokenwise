...
- [ ] **Switch figure.** Claude Code's own confirmation for a warm-cache model switch says the history gets re-read, and gives no size or cost. A PreModelSwitch hook shows both above the threshold and returns no decision, so every switch goes ahead or not as it would without the plugin. Claude Code shows the line with the command's result once a switch applies, and not when it is cancelled, so the line gives what the next message will re-send. It is verified once in a live session. The hooks reference lists no event for effort changes, so those rely on Claude Code's confirmation.
