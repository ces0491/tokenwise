...
A Claude Code plugin that gets more work out of a Claude subscription for the least effort from its user. It checks the default new sessions start on, warns before an action re-sends a large conversation, and recommends the model and effort for the work at hand, each with the evidence behind it.
