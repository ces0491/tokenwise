...
/** VAT for one line. */
export function lineVat(line, region) {
  return vatOn(lineNet(line), region);
}

/** Net, VAT and gross totals for an invoice. */
export function invoiceTotals(invoice) {
  const net = sum(invoice.lines.map(lineNet));
  const vat = sum(invoice.lines.map((line) => lineVat(line, invoice.region)));
