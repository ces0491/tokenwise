...
import { creditTotals } from './creditnote.js';
...
/** Net, VAT and gross totals for an invoice or a credit note. VAT is calculated once on the net. */
export function invoiceTotals(doc) {
  if (doc.kind === 'credit') return creditTotals(doc);
  const net = sum(doc.lines.map(lineNet));
  const vat = vatOn(net, doc.region);
