...
Route from this file and what the user has said. Do not read other files, run commands or check settings to answer, and keep the reasoning short. The answer stays in the user's context and is paid for on every later call, like anything else that enters it.
...
- Boundary: whether to `/clear` or `/compact` first, and what the switch costs if not. If this route came in the middle of a phase, say that this answer stays in context until the next `/clear`.
