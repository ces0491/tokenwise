...
function verdicts() {
  ...
  const c12 = ['implement', 'debug', 'multi', 'multi-large'].filter((c) => need(`${c}-opus55-medium`, `${c}-sonnet-medium-c12`)).map((c) => {
    ...
    const cpc = (x) => (x.costPerCompleted == null ? 'never completed' : usd(x.costPerCompleted));
    const ratio = so.costPerCompleted != null && o.costPerCompleted != null ? ` (${(so.costPerCompleted / o.costPerCompleted).toFixed(2)}x)` : '';
    return { v, n: Math.min(o.n, so.n), note: `${c}: Sonnet ${v}, ${so.passes}/${so.n} at ${cpc(so)} per completed task against Opus 5.5 ${o.passes}/${o.n} at ${cpc(o)}${ratio}` };
  });
  if (c12.length) {
    const saves = c12.filter((r) => r.v === 'saves').length;
