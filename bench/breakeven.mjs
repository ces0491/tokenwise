...
if (import.meta.main ?? (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))) {
  ...
  if (process.argv.includes('--check')) {
    if (committed !== svg) { process.stderr.write('docs/breakeven.svg does not follow from the committed data. Run node bench/breakeven.mjs and commit it.\n'); process.exit(1); }
    process.stdout.write('docs/breakeven.svg follows from the committed data.\n');
  } else {
    // An unchanged chart is left alone, so a checkout with CRLF line endings does not show it modified.
    if (committed !== svg) fs.writeFileSync(OUT, svg);
    process.stdout.write(`${tables(data)}\n\n${committed === svg ? 'docs/breakeven.svg is already up to date.' : `Wrote ${path.relative(ROOT, OUT)}.`}\n`);
  }
}
