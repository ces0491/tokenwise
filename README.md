...
Those task costs leave out the skill itself, so its cost is measured separately. Loaded and unused, it adds 129 tokens of context. It runs on your session's model and effort: a route in a session already under way cost $0.04 asked from Sonnet 5 at medium, $0.10 from Opus 5 at high and $0.14 from Opus 5 at xhigh. The first route left 620 to 826 tokens behind, carried on every later call, and routing just before a `/clear` carries nothing. For a single small chore, asking can cost about what the cheaper model saves.
