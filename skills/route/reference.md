...
Two kinds of evidence appear here. The token measurements below are observational, parsed from real Claude Code transcripts on one machine by `bench/context-profile.mjs`. The per-row model and effort recommendations were tested separately on a benchmark of graded tasks; that is in `../../docs/findings.md`, and it falsified seven of the twelve claims it set out to test. Where the two disagree, the benchmark wins for anything it covers, which is task-level model and effort choice on a small codebase. It does not cover long-context sessions, which is what the numbers below describe.
...
From the API pricing page, fetched 23 September 2026. Output costs 5x input on every current model, and thinking is output. A cache read costs 0.1x input (0.05x on Opus 5.5, 0.025x on Fable 5.1). A cache write costs 1.25x input on the 5-minute cache and 2x on the 1-hour cache. Fable 5.1 is 5x Sonnet 5 and 10x Haiku 4.5 on input and output, and 1.25x and 2.5x on cache reads. Opus 5.5 is 2x Sonnet 5 on input, cache writes and output, and the same price on cache reads. Opus 5, which the bench ran and the models overview now lists as a legacy model, is 2.5x Sonnet 5 on every class. Models from Claude 4.7 on use a newer tokenizer that the page says produces about 30% more tokens for the same text. Haiku 4.5 predates 4.7, so its token counts are not directly comparable with the other current models'.
...
From the Claude Code prompt caching docs: "Each model has its own cache. Switching models recomputes the entire request even when the content is identical." And: "on most models, each effort level has its own cache, so changing effort mid-session recomputes the entire request. On Opus 5.5 and Fable 5.1 with an API key or a Claude subscription, the cache stays intact by default." Claude Code asks for confirmation on `/model` only while the cache is warm (from v2.1.238), and asks before an effort change while the cache is warm, except on those two models, where it "applies the new level without asking".
...
From the Claude Code skills docs: "When you or Claude invoke a skill, the rendered `SKILL.md` content enters the conversation as a single message and stays there across later turns." A second invocation with different arguments appends the full content again, and auto-compaction re-attaches invoked skills after its summary, up to a token budget. `context: fork` runs a skill in a subagent instead: Claude Code "gives it the skill content as its prompt. The subagent doesn't see your conversation history".

This skill runs forked for that reason. Measured on Opus 5 at xhigh effort (`../../docs/findings.md`), a route that ran in the conversation left 9.6K tokens behind for every later call, most of it the answer and its thinking. Forked, a 1.4.0 route left 873, the answer alone. The cost is that the skill routes from the description it is given and cannot see the conversation or its context size.
...
From the API effort docs: `high` is the default on every model that supports effort except Opus 5.5, which defaults to `medium`. The advice for Opus 5 and Fable 5.1 is to start at `high`. For Opus 5.5 the page says to "Run an effort sweep on your own evals rather than carrying settings over from an earlier model". The skill's Opus rows were measured on Opus 5, so by that advice their effort levels are a starting point on Opus 5.5 rather than a result. The docs describe `xhigh` as extended capability for long-running agentic and coding work, `max` as the maximum with no limit on token spending, and `low` as the most efficient level, for simpler tasks such as subagents. Lower effort also means fewer and terser tool calls. Claude Code's model configuration docs give `high` as its default on every model that supports effort, except Opus 5.5, which defaults to `medium`, and Opus 4.7, which defaults to `xhigh`. They add that "The effort scale is calibrated per model, so the same level name does not represent the same underlying value across models."
...
On cost, the same page: "A workflow spawns many agents, so a single run can use meaningfully more tokens than working through the same task in conversation." Sessions with ultracode on skip the large-workflow warning, "because turning ultracode on already opts you in to large runs". The `/config` size guideline is advice to Claude on how many agents to aim for; `medium`, fewer than 10 agents, is the default from v2.1.219, and `small`, fewer than 5, on a Pro plan from v2.1.271. A prompt that calls for a different scale overrides it. The runtime caps a run at 16 concurrent agents, fewer when Claude Code has fewer CPUs available, and 1,000 agents in total. The page also suggests asking Claude "to use a smaller model for stages that don't need the strongest one when you describe the task". None of the pages under Sources gives a usage or price figure for ultracode, and the costs page does not mention it.
