...
Corrections from the review before tagging. Setup no longer recommends a change from Sonnet 5 at xhigh, a changed recommendation, so this is a minor release. No row of the routing table changed.

- **Setup holds every setting to one rule.** It recommends Sonnet 5 at medium only from a setting where Sonnet cost at most 0.7 times as much on every task the bench ran both: Opus 5 at medium, high or xhigh, and Fable 5.1 at xhigh. 1.4.0 also recommended it from Sonnet 5 at xhigh, which cleared the band on the implement task, at 0.51 times, and not on the bug fix, at 0.80 times on one run. Sonnet 5 at high came to 0.72 times on its one task, which 1.4.0 gave as 0.71 from rounded figures.
- **Setup reads effort as Claude Code does.** A `maxEffortLevel` cap, top-level or in a model's `modelSettings` entry, lowers the level a model runs at. 1.4.0 ignored caps, so it could report Opus 5 at high for a session capped at low and recommend a change from a setting the bench never ran. `default` is matched in any case.
- **Setup says what else decides where sessions start.** The skill lists `overrides` on every path, where 1.4.0 listed them only beside a recommendation, and `overrides` names `ANTHROPIC_DEFAULT_MODEL`, the `ANTHROPIC_DEFAULT_*_MODEL` alias variables and project effort caps. `show` reports the installed Claude Code's version, and on one older than 2.1.280, where the aliases resolved to other models, setup recommends nothing and says to update. A user an earlier setup moved to Sonnet 5 at medium is told about C12 and `restore`. Haiku, which takes no effort level, has its own wording.
- **The C12 verdict counts runs.** `bench/SCOPE.md` says "passes at least as many runs", and the code compared pass rates, which differ when a cell is short a run. The evidence line also says when the cells span more than one Claude Code version. No verdict changed.
- **Figures and wording.**
  - The findings and the setup skill said Opus 5.5 took fewer API calls. The bench records turns, so they say turns.
  - The methodology gave the C12 sessions' median overhead as 51K. It is 45K: 38K to 39K on Opus 5.5 and 51K on Sonnet 5.
